...
const { mapFiltersForLinkedIn } = require('./filters');
const { normalizeLinkedInItems } = require('./normalize');

const ACTOR_ID = 'bebity/linkedin-jobs-scraper';
const ACTOR_NAME = 'LinkedIn scraper';
...
function getApifyClient() {
  ...
  if (!token) {
    throw new Error('APIFY_API_TOKEN is required to run the LinkedIn scraper.');
  }
...
async function runLinkedInScraper(filters) {
  const client = getApifyClient();
  const actorInput = mapFiltersForLinkedIn(filters || {});
  ...
  return normalizeLinkedInItems(items);
}
